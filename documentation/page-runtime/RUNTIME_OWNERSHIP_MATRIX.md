...
| Main state socket | `database.js`, `routes/state.js` | jQuery ready / reconnect | activity hash, WebSocket | socket `open` plus separate `state-websocket-liveness`: `checking`, `healthy`, `degraded`, or `stopped` | 18-second heartbeat; pong stale after 45 seconds; reconnect backoff capped at 15 seconds and reset to 1 second after successful open | `RETAIN`; application heartbeat and reconnect recovery implemented; stale-pong is diagnostic only; add offending-state paths to sync diagnostics |
| Initial state sync | `database.js`, `routes/state.js`, `initial-state-protocol.js`, `page-runtime.js`, coordinator adapter | `watch` -> `initial-state-result` | identity, activity hash, MongoDB | explicit `found`, `empty`, `failed`, or `invalid-request`; reconnect records separate `state-resynchronization` | 15-second `XR-STATE-INITIAL-101`; late valid recovery allowed; detailed watchdog ownership still partly legacy-driven | `IMPLEMENTED / RECONCILED` for current scope; `unauthorized` remains future security work; fallback/retry policy remains deferred |
| Activity bootstrap | coordinator-owned one-shot trigger in `main.js`; state-dependent release coordinator-owned through `activity.js`; initialization body remains in `activity.js` | coordinator receives document-ready and invokes guarded `.activity()` once; activity registers a release request when its initial-state consumer becomes available | initial state, activity initialization request; generic `fetchData()` queue remains database-owned | `activity-bootstrap-trigger` succeeds after invocation; `activity-initialization-release` succeeds after the initialization body runs; activity component becomes initialized | guarded legacy fallbacks if coordinator ownership cannot be configured or requested; duplicate invocation and duplicate release requests ignored | `IMPLEMENTED FOR PHASE 1`; trigger and state-dependent release ownership transferred, internal activity behavior deliberately remains component-owned |
...
| Runtime support policy/UI | `page-runtime-support-snapshot.js`, `page-runtime-support-policy.js`, `page-runtime-support-ui.js` | support snapshot/change notification | runtime lifecycle state, stable support taxonomy | unified banner plus report modal; clears/replaces on recovery | inherits subsystem policy/deadline | `IMPLEMENTED FOR PHASE 1`; keep support codes stable and recovery actions separate from codes |
| Privacy-safe support report | `page-runtime-support-report.js` | student **Generate & Copy Report** action | support snapshot, bounded runtime inspection, browser metadata | schema-v1 allowlisted report copied or exposed for manual copy | max 30 recent events; event details excluded | `IMPLEMENTED FOR PHASE 1`; preserve privacy allowlist; consider schema-v2 enrichment only when diagnostically justified |
| Support correlation | `page-runtime.js`, `database.js`, `sagemath.js`, `app.js`, `routes/state.js` | page runtime creation, state watch, Sage auth/service | non-secret `supportTraceId` | report-to-Xronos-log correlation | trace validation length/ASCII constraints | `IMPLEMENTED ON XRONOS`; diagnostic-only, not auth and not Sage cache identity; SageCell-side logger source-ready but deployment deferred |
| Runtime support contact | `config.js`, `views/page.pug`, `page-runtime-support-ui.js` | server startup/config render | `XRONOS_SUPPORT_EMAIL` | configured email shown in report modal; generic fallback when unset | none | `IMPLEMENTED`; deployment-specific value stays outside Git |
