...
| Activity bootstrap | coordinator-owned one-shot trigger in `main.js`; legacy implementation in `activity.js` | coordinator receives document-ready and invokes guarded `.activity()` once | initial `fetchData()` remains legacy-owned | trigger task completes after invocation; activity component completes after callback body | guarded legacy fallback if coordinator ownership cannot be requested; duplicate invocation ignored | `ACTIVE MIGRATION`; trigger ownership transferred, internal bootstrap remains legacy-owned |
