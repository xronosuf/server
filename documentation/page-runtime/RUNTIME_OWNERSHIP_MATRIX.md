...
| Math answer | `math-answer.js`, `main.js`, `page-runtime.js` | MathJax `New Math` connection and activity bootstrap | state, parsed answer model, optional validator | initial pass reports `initial-math-answers`: `settled`, `degraded`, or `not-required`; submissions still end implicitly | no separate initial-answer deadline; terminal state currently follows the initial MathJax process | `RETAIN`; add submission transaction boundary and permit verified late connection recovery without erasing failure history |
