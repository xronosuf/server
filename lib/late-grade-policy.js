...
function passbackWindowEnd(bridge, fallbackDays) {
    ...
    var due = timeValue(bridge && bridge.dueDate);
    var shellFallback = timeValue(
        bridge && bridge.fallbackGradeSyncEndAt
    );
    var configuredCutoff = bridge && bridge.gradeSyncCutoff;

    /*
     * New LMS-shell instructor settings materialize the resolved policy onto
     * each LTI bridge so background grade workers do not need a request/session
     * or an asynchronous settings lookup.  The shell document remains the
     * authoritative source and refreshes these bridge fields on launch/change.
     */
    if (configuredCutoff === 'due-date') {
        if (due !== null) {
            return {
                time: due,
                source: 'xronos-due-date-setting'
            };
        }
    ...
    if (configuredCutoff === 'late-policy') {
        if (until !== null) {
            return {
                time: until,
                source: 'canvas-until'
    ...
    /*
     * Backwards-compatible behavior for bridges that predate the settings
     * rollout and have not yet been refreshed by a validated LTI launch.
     */
    ...
    if (until !== null) {
        return { time: until, source: 'canvas-until' };
    }
    if (due !== null) {
        return {
    ...
    return { time: null, source: 'no-end-date' };
}
...
function nextLateIntervalBoundary(bridge, now) {
    ...
    now = now === undefined ? Date.now() : now;
    if (due === null || interval === 0) return null;
...
function lateBoundaryDecision(bridge, now, bufferMs) {
    ...
    var remaining;
    now = now === undefined ? Date.now() : now;
    bufferMs = bufferMs === undefined ? DEFAULT_BOUNDARY_BUFFER_MS : bufferMs;

    if (!bridgeIsLate(bridge, now)) {
        return {defer: false, reason: 'not-late', retryAt: null};
    }
    boundary = nextLateIntervalBoundary(bridge, now);
    if (boundary === null) {
        return {defer: false, reason: 'no-boundary', retryAt: null};
    }
    ...
    return {defer: false, reason: 'safe-interval', retryAt: null};
}
...
function normalizedRawScore(resultTotalScore, pointsPossible) {
    ...
    if (total === null || possible === null || possible <= 0) return null;
...
function normalizeObservation(observation) {
    ...
    if (raw === null || effective === null || !isFinite(intervals) ||
        intervals <= 0 || Math.floor(intervals) !== intervals ||
...
function deriveExactFloor(valid, deduction) {
    var floorEvidence = valid.filter(function(observation) {
        var floorless = observation.rawScore - observation.lateIntervals * deduction;
        return observation.effectiveScore > floorless + EVIDENCE_TOLERANCE;
    });

    if (floorEvidence.length === 0) {
        return {exact: false, value: null, evidenceCount: 0, inconsistent: false};
    }

    var floor = floorEvidence[0].effectiveScore;
    var inconsistent = floorEvidence.some(function(observation) {
        return Math.abs(observation.effectiveScore - floor) > EVIDENCE_TOLERANCE;
    });
    ...
    if (!inconsistent) {
        inconsistent = valid.some(function(observation) {
            ...
            return Math.abs(expected - observation.effectiveScore) > EVIDENCE_TOLERANCE;
        });
    }
...
function deriveContextPolicy(observations) {
    var valid = (observations || []).map(normalizeObservation).filter(function(observation) {
        return observation !== null;
    });

    if (valid.length === 0) {
        return {
            usable: false, exact: false, reason: 'no-valid-context-evidence',
            deductionPerInterval: null, gradeFloor: null,
            gradeFloorExact: false, floorEvidenceCount: 0, evidenceCount: 0
        };
    }
    ...
    var definitelyNotFloored = valid.filter(function(observation) {
        return observation.effectiveScore > minimumEffective + EVIDENCE_TOLERANCE;
    });
    ...
    var inconsistent = definitelyNotFloored.some(function(observation) {
        return Math.abs(observation.deductionLowerBound - deduction) > EVIDENCE_TOLERANCE;
    });

    if (!inconsistent) {
        inconsistent = valid.some(function(observation) {
            var floorless = observation.rawScore - observation.lateIntervals * deduction;
            if (observation.effectiveScore > minimumEffective + EVIDENCE_TOLERANCE) {
                return Math.abs(floorless - observation.effectiveScore) > EVIDENCE_TOLERANCE;
            }
            return floorless > observation.effectiveScore + EVIDENCE_TOLERANCE;
        });
    }

    if (inconsistent) {
        return {
            usable: false, exact: false, reason: 'inconsistent-context-evidence',
            deductionPerInterval: null, gradeFloor: null,
            gradeFloorExact: false, floorEvidenceCount: 0, evidenceCount: valid.length
        };
    }
    ...
    if (floor.inconsistent) {
        return {
            usable: false, exact: false, reason: 'inconsistent-context-evidence',
            deductionPerInterval: null, gradeFloor: null,
            gradeFloorExact: false, floorEvidenceCount: floor.evidenceCount,
            evidenceCount: valid.length
        };
    }
    ...
    return {
        usable: true,
        exact: true,
        reason: floor.exact ? 'exact-context-deduction-and-floor' : 'exact-context-deduction',
...
function localObservationDecision(options) {
    ...
    if (!observation || canvasScore === null || candidateRawScore === null ||
        !isFinite(currentLateIntervals) || currentLateIntervals <= 0 ||
        Math.floor(currentLateIntervals) !== currentLateIntervals) return null;

    if (observation.source && observation.source !== AUTHORITATIVE_POLICY_SOURCE) return null;
    if (!sameEffectiveScore(canvasScore, observation.effectiveScore)) return null;

    if (candidateRawScore <= observation.rawScore + EPSILON) {
        return {allow: false, reason: 'candidate-not-higher-than-local-evidence', predictedFloorlessScore: null};
    }
    ...
    return {
        ...
        reason: predictedFloorless + EVIDENCE_TOLERANCE >= canvasScore
...
function contextPolicyDecision(options) {
    ...
    if (!policy.usable || !policy.exact ||
        !isFinite(Number(policy.deductionPerInterval)) || canvasScore === null ||
        candidateRawScore === null || !isFinite(currentLateIntervals) ||
        currentLateIntervals <= 0 || Math.floor(currentLateIntervals) !== currentLateIntervals) {
        return null;
    }

    var deduction = Number(policy.deductionPerInterval);
    var predictedFloorless = candidateRawScore - currentLateIntervals * deduction;
    var floor = policy.gradeFloorExact ? finiteScore(policy.gradeFloor) : null;
    var predictedEffective = floor === null ? predictedFloorless : Math.max(predictedFloorless, floor);

    return {
        allow: predictedEffective + EVIDENCE_TOLERANCE >= canvasScore,
        reason: predictedEffective + EVIDENCE_TOLERANCE >= canvasScore
            ? (floor !== null && predictedEffective > predictedFloorless + EVIDENCE_TOLERANCE
...
function latePassbackDecision(options) {
    ...
    if (candidateRawScore === null) return {allow: false, reason: 'invalid-candidate-score'};
    if (!options.canvasHasResult) {
        return {
            allow: candidateRawScore > 0,
            reason: candidateRawScore > 0 ? 'canvas-has-no-result' : 'zero-first-result'
        };
    }
    if (canvasScore === null) return {allow: false, reason: 'invalid-canvas-score'};

    var contextDecision = contextPolicyDecision(options);
    if (contextDecision) return contextDecision;

    if (options.contextPolicy && options.contextPolicy.reason === 'inconsistent-context-evidence') {
        return {allow: false, reason: 'inconsistent-context-evidence'};
    }

    var localDecision = localObservationDecision(options);
    if (localDecision) return localDecision;
    return {allow: false, reason: 'insufficient-late-policy-evidence'};
}
